// client/src/hooks/useVideoProgress.ts - Optimized for real-time updates
...
const MIN_VIEWING_TIME = 0.2; // Very small to capture even brief views
...
const mergeIntervals = (intervals: TimeInterval[]): TimeInterval[] => {
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        if (current.start <= lastMerged.end + 0.2) {
...
const calculateProgressPercentage = (intervals: TimeInterval[], videoDuration: number): number => {
    ...
    merged.forEach((interval, idx) => {
        console.log(`Interval ${idx}: ${interval.start.toFixed(2)}s - ${interval.end.toFixed(2)}s (${(interval.end - interval.start).toFixed(2)}s)`);
    });
...
export const useVideoProgress = ({ videoId, videoDuration }: UseVideoProgressProps): UseVideoProgressReturn => {
    ...
    const startTracking = useCallback((currentTime: number) => {
        console.log(`Started tracking at ${currentTime}`);
    ...
    const stopTracking = useCallback((currentTime: number) => {
        console.log(`Stopped tracking at ${currentTime}`);
        if (trackingStartTime.current !== null && lastTrackedTime.current !== null) {
            ...
            if (end - start >= MIN_VIEWING_TIME) {
                ...
                console.log(`Adding interval: ${start.toFixed(2)}s to ${end.toFixed(2)}s`);
    ...
    const saveProgress = useCallback(async () => {
        ...
        // Only save if we have new intervals
    ...
    }, [videoId, videoDuration, progress]);
